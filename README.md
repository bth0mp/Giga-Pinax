...
- **Research** — resolve a reference from American Numismatic Society and Nomisma data, or read every catalogue reference out of a pasted lot description or a right-clicked selection. An ambiguous reference offers choices instead of inventing an answer.
- **Prices** — a median of recent acsearch sales, and a separately labelled median of CoinArchives public results. Each shows its own query, sample and coverage; you can exclude a sale and watch the figures follow. Currencies and providers are never pooled.
