...
- **Want list** — the types you are looking for, each with an optional maximum price in one currency, lowest grade and notes. A popup card of a wanted type and an **Upcoming** acsearch lot citing it carry a **Wanted** pill with the want's terms, and a captured lot drafted in the workspace says **On your want list**; a won coin of the type can mark the want found. Wants are matched by the catalogue rules the lookup reads references with (`RIC I (second edition) Nero 306` is `RIC I² Nero 306`, `306a` is not), only for a reference that names one type — never a bare RIC number, a list of candidates or a text no catalogue reads — and nothing is searched for or sent anywhere.
