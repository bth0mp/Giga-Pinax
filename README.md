...
- **Prices** — a median of recent acsearch sales, and a separately labelled median of CoinArchives public results. Each search looks for the reference as a phrase dealers actually write, counts how many results cite it (`39 of 55 results cite Price 23`), in the spellings houses write (`Price 23`, `Price-23`, `RIC II, 118`, `RIC: 118`), and keeps the rest out of the median; you can switch that filter off, narrow to one denomination, read a median per dealer grade (English, German, French, Italian, Spanish and Dutch grades are read, house spellings such as `GVF`, `Fdc`, `Stgl.` and `prfr.` included, a grade written straight behind the weight such as `3,21 g TTB` or `g 17,10 BB`, a range with a spaced plus such as `Zeer fraai +/prachtig` read as its lower half, and the Spanish `S/C` behind a weight or a closing remark, never behind a described reverse) and a median per year of the fetched sales, exclude a sale and watch the figures follow. Lots on the acsearch page that are not sold yet are listed as **Upcoming**, filtered the same way, each with a **Watch** button that saves it to the watchlist in one step. Each panel shows its own query, sample and coverage. Currencies and providers are never pooled.
...
- **Calculator** — in USD, EUR, GBP, CHF, AUD, CAD, CZK, DKK, HKD, HUF, JPY, NOK, PLN or SEK, each in its own minor units (whole yen) and never converted: hammer plus buyer's premium, or the highest affordable hammer from a budget, including VAT on the premium, a live-bidding platform's fee on the hammer, import VAT or duty on hammer, premium and shipping for a coin crossing a border (Settings can start it at your usual rate for a sale in another currency), shipping, percentage and fixed payment fees, and either a fixed bid increment or the increment ladder you copied from a house's own terms; the median the popup just drew is offered as the hammer, and on a saved coin's Bid tab as its maximum, for this browser session only.
...
- **Want list** — the types you are looking for, each with an optional maximum price in one currency, lowest grade and notes. A popup card of a wanted type and an **Upcoming** acsearch lot citing it carry a **Wanted** pill with the want's terms, and a captured lot drafted in the workspace says **On your want list**; a won coin of the type can mark the want found, from the Want list or straight from the coin's **Save outcome**. Each want on the page says since when it is wanted, lists the coins of its type on your watchlist with their bid and sale, and offers **Look up** and **Search acsearch** for the type. Wants are matched by the catalogue rules the lookup reads references with (`RIC I (second edition) Nero 306` is `RIC I² Nero 306`, `306a` is not), only for a reference that names one type — never a bare RIC number, a list of candidates or a text no catalogue reads — and nothing is searched for or sent anywhere until you choose **Look up** or **Search acsearch**. A want is checked against the bundled catalogue when you save it: one it holds under another title is saved under that title (`RIC I Nero 306` becomes `RIC I² Nero 306`, the only RIC I it holds), and one no card could ever match is refused with the reason. `RIC II Trajan 253` wants both of OCRE's `253 (aureus)` and `253 (denarius)`.
