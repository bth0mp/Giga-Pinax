...
export async function mountWorkspace({ background = null, hash = '', confirmAnswers = [], language = 'en-US', wide = false, catalogue = null } = {}) {
  ...
  return {
    ...
    async openCoin(title) {
      // A row is the coin's button, beside its compare box (G-14), and in a month's group where the list has them (K-06).
      const row = $('lot-list').querySelectorAll('.coin-row').find((item) => item.textContent.includes(title));
