...
Nomisma concept labels and classes are absent from every one of these exports. A separate, checked-in aggregate RDF snapshot supplies labels and classes for the 272 Nomisma concepts OCRE references as an authority or obverse portrait; it was filtered against OCRE's own concepts and names RIC's people and nobody else's, so only an OCRE card reads it. A CRRO, PELLA or SCO card resolves an identifier from the label cache that the online Nomisma lookups fill, and leaves it unresolved where the cache cannot — exactly as the online card for the same record leaves it. Nothing is guessed and no label is taken from a source that was not asked about that corpus.

No local lookup resolves a label of its own: it makes no request at all. So a card built from the bundled data shows the identifier itself — `antiocheia_syria`, `denomination_b_sco`, `ar` — wherever no earlier online lookup has put a name for it in the cache. This is what an OCRE card has always done for its denomination, mint and material; the three new corpora do the same for those and for the authority, since the people snapshot names nobody outside RIC. The gap is the labels, never the record: `tests/local-catalogue.test.mjs` compares the local card with `toCard` over the same record's JSON-LD, field by field, both with the labels resolved and with none, and they are equal either way. Closing the gap would mean either bundling a Nomisma snapshot for these corpora — a second source of data, under CC BY 3.0 rather than ODbL — or resolving labels over the network on a local hit, which would cost the lookup the property that makes it local. Neither was done here. `ric-people.js` includes the 214 concepts that Nomisma classifies as `foaf:Person`; it excludes 58 deities, personifications, groups and other concepts. Canonical names are English preferred labels. English and Latin preferred or alternate labels are search aliases. Missing legends, descriptions and dates remain absent.
...
Each corpus is loaded on its own and only when it is asked about: a popup that has just opened reads nothing, a Bopearachchi reference reads nothing, and a Price lookup never touches OCRE's files. A reference that names its record outright — `SC 1266.2` is `sc.1.1266.2` — opens the record from the metadata and one shard, without reading an index at all. A RIC reference reads the number index and the searchable index; a Crawford or Price reference reads the searchable index and is matched against whole titles, first as an exact title and then against the near misses of its own group, through the same `pickMatch` and `inGroup` rules the online search results go through. Anything short of an exact answer is a local miss, and the lookup falls back online rather than reporting that the type does not exist. Local lookups require neither a network service nor ANS permission. Missing data can use the existing online provider with access, and the explicit **Check online** action can request that access. Results describe their bundled source; an absent local match does not establish that a type does not exist.
