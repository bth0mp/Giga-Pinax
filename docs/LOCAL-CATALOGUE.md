...
Wikidata also lists, beside a city's names, things that are not one, and those are refused by concept in `NOT_A_NAME` in `scripts/import_people.py`, each with the reason beside it: the title of an encyclopaedia article about the place ("history of Pavia"); a nickname ("the Eternal City", "Caput Mundi", "Pearl of the Mediterranean"); and a whole different place (Longpré-lès-Amiens is a commune of its own). A nickname has to go because a heading naming no ruler is read for the earliest mint spelling in it, so a Trier coin sold with a line of prose about the Eternal City would have been filed under Rome. An alias of one or two characters is a code rather than a name — Wikidata lists "RM", Rome's Italian province code — and is dropped as a class.

Of the 21 mint concepts in the tracked snapshots, 18 carry a modern name that differs from the section RIC files them under. Twenty of the 21 link to a Wikidata item — Thessalonica is the one that links to none, and keeps the names Nomisma itself publishes — and Ticinum and Treveri link to two apiece. The three left with nothing are Aquileia, which already is the name on the map, and Mediolanum and Ticinum, which are not.

**"London", "Lyon", "Lyons", "Milan" and "Pavia" are still not recognised**, and this is a limit of the sources rather than an oversight. The Wikidata items Nomisma links Londinium, Lugdunum, Mediolanum and Ticinum to are the *Roman* city — Q927198, Q665, Q729978 and Q28215083 — and every one of them is titled by the Latin name in every language kept: Wikidata's English label for Q665 is "Lugdunum", not "Lyon". Each item does carry a statement pointing at the town standing there now, but a statement is not a name, no single property says it for all four (`P1366` for Lugdunum and Mediolanum, `P276` for Londinium, `P131` for Ticinum), and following one would file a coin under a place nobody published as that mint's name. Nothing was invented, so a lot written "London" or "Lyon" is still looked up by RIC's own spelling — Londinium, Lugdunum, Mediolanum, Ticinum — which reaches all four. "Roman London", which Wikidata does publish as a name of Q927198, reaches Londinium.

A lot heading that names a mint and nobody else ("Londinium. RIC 12", "Arles mint") is read as that mint's section, with the volumes the section implies. A heading that names a ruler as well is the ruler's, exactly as before: the mint only says where the coin was struck, and a ruler volume's number would otherwise be thrown away for it. The mint never joins the rulers a lot row carries, so nothing asks OCRE's portrait facet for a place.
...
The Wikidata snapshot is refreshed from that file and from nothing else: the `matches` links in it decide which items are asked for, one request each, under the same User-Agent. A mint linking to no Wikidata item is named in the run's own output rather than passed over in silence, and only the items' labels and aliases are written down. Wikidata dedicates them to the public domain under CC0 1.0, and the snapshot, `extension/data/NOTICE.txt` and the generated `extension/ric-people.js` header all say so:
...
Both mint snapshots travel together: a regeneration stops if the Wikidata one no longer holds an item the Nomisma one links to, rather than losing that item's names without saying so.
