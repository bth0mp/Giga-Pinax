...
Unsaved capture drafts, which hold the captured page text and what the page stated about the lot's sale (the price its offer gives, when it closes or its auction starts, a photo link and the provenance it lists), and a short ledger of recent write requests are also kept in that storage so a save is not repeated or lost. A reference you have typed is held in the browser's extension session storage while a permission prompt closes the popup, and is discarded when the browser session ends.

The research popup also keeps a few display settings in the extension page's own `localStorage`, so it opens the way you left it: your last six lookups, up to 50 price-search phrases you have edited, the research form's catalogue, number, volume, ruler or mint, currency and sales period, the light or dark theme, whether **Show specimen photos** is switched on, a cache of the Nomisma names an online card has already been given, and a copy of the default currency the popup can read before the background answers it. None of it is part of a backup or of the raw export, and all of it goes when the extension's data is removed.

## Network requests

RIC, Crawford (RRC), Price, Seleucid Coins (SC), CPE (Lorber's Coins of the Ptolemaic Empire) and Newell (Demetrius Poliorcetes) reference lookups first check the OCRE, CRRO, PELLA, SCO, PCO and AGCO data packaged inside the extension. A local match does not transmit the reference to ANS or Nomisma, unless you have switched on **Show specimen photos** (below), which asks Nomisma for photographs of the matched type. If the bundled data cannot answer the lookup, the extension can use online catalogue services with provider access; for a CPE or Newell (Demetrius Poliorcetes) reference that is one request to `numismatics.org` for the record its number names. If that access is disabled, an explicit **Check online** action requests it. Bopearachchi references are not bundled and always use the online services, because the citation that identifies a Bopearachchi type is only in the online records. A local match makes no Nomisma request for its names either: the English Nomisma.org names for every authority, denomination, mint, material and portrait the bundled records carry are packaged with them, under CC BY 3.0, so a local card is named from inside the package. Ruler names on RIC cards come from the Nomisma snapshot in `ric-people.js`, which takes precedence over the bundled label file. A concept Nomisma publishes no English name for shows as the identifier the record carries, rather than a guess. Auction-price research is independent of the type record: the acsearch price search starts with every lookup once acsearch access is granted, and does so whether the type record came from inside the package or from online.
...
- Nomisma.org at `nomisma.org` for public numismatic terms and labels, and, only when **Show specimen photos** is switched on in Settings, for the list of museum coins photographed for the one type a card shows (the request names that type, never a search phrase or a record);
...
**Show specimen photos** in Settings is off by default. When you switch it on, a card for one catalogue type — whether its record came from inside the package or from online — asks Nomisma.org once which museum coins of that type have been photographed, and shows up to three of them, each with the collection that holds it and a link to its page. No request is made for a list of choices or for a reference without type data. The photos load straight from the museums' and libraries' own image servers, which receive an ordinary image request with connection data such as your IP address; each image is lazy-loaded with no referrer, and an image address that is not HTTP or HTTPS is never loaded. Nothing about them is kept: not the answer, an image address or a photo, and they are not part of Copy summary, a record or a backup. Switched off, no specimen request is made at all.
...
Core research providers use HTTPS. Explicit collector-supplied photo URLs, and a museum's specimen photo address, may use HTTP; an HTTP image request is not encrypted in transit. Browser extension storage is protected by the browser profile and device, but it is not an encrypted vault. Avoid saving unrelated secrets or credentials in notes.
