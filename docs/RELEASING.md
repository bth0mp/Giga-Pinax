...
4. Push the commit, then push the tag `vx.y.z`. The `release` workflow first refuses a tag whose version disagrees with `manifests/brave.json`, then runs both test suites, the build and the Firefox lint, creates the release as a **draft** if it does not exist, and uploads the five ZIPs with `gh release upload --clobber`. It never publishes, and it refuses to upload into a release that has already been published. The first `v0.32.0` tag will be the first time this workflow has ever run, so watch the run itself as well as its output.
