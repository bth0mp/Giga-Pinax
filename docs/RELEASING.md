...
The bundled catalogue data is refreshed separately: `.github/workflows/refresh-data.yml` downloads the ANS exports once a month (or when started by hand), imports the ones that changed, tests and builds them, and opens a pull request — it never pushes to `main` and never publishes anything. Merge such a pull request, and do what its checklist asks, before cutting the release that should carry the new data; [LOCAL-CATALOGUE.md](LOCAL-CATALOGUE.md#the-monthly-refresh) describes it.
