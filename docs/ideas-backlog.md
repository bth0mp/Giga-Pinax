...
7. **Deferred review minors:** from the v0.7 fix review — if focus is on a chip when an unrelated lookup finishes, it jumps to the first chip (refocus the chip with the same corpus + id instead); on the no-access path "Found X." can be announced twice. From the v0.7 review — a Recent chip whose label doesn't parse (e.g. an OCRE card whose label fell back to its id) leaves the guided fields stale, so the automatic acsearch term comes from the previous reference; with host access revoked in Firefox a chip lookup shows the network message instead of the permission message; whitespace-only `id`/`label` pass restore. (The v0.6 review's three minors shipped in v0.7.) Older: candidate list accessible name; `aria-invalid` only for not-found; drop dead `.try-it a` rules; read the label cache once per popup; `resolveLabels` option defaults.
