...
Result 2026-09-11 at `948f2d0`, in-app Chromium at 400×600 on `http://localhost:8779` (never loaded before), live numismatics.org: `RIC I² Nero 306` + submit → fields `RIC` / `I (2nd edition)` / `Nero` / `306`, card `RIC I (second edition) Nero 306`, term `Nero 306`; `Crawford 44/5` on `change` → catalogue RRC, number `44/5`, RIC fields hidden, label `Crawford number`, then card `RRC 44/5`, term `Crawford 44/5`; `Price 23` → `Price 23`; `Sear 1234` → the one-box error, result hidden, 0 numismatics.org requests; editing the number field and changing the catalogue each cleared the box; `RIC II.3 Hadrian 12` → fields `II, Part 3` / `Hadrian` / `12`, one suggestion `RIC II, Part 3 (second edition) Hadrian 12`; one-box text not in stored preferences; 400 px; console clean. Pass, with one gap confirmed: when a required guided field is empty and the box does not parse, the browser's native validation blocks submit, so the one-box error never shows.

After the polish fix (`a72d79a`), on another never-loaded origin (`http://localhost:8780`): the form has `novalidate`; empty number + `Sear 1234` → the one-box error, `#quick-reference` `aria-invalid="true"`, `#reference-number` unmarked, `aria-describedby="quick-help form-error"`, 0 requests; empty box + empty number → form invalid, native prompt, no one-box error, 0 requests; `RIC I 2 Nero 306` → the one-box error; `“Crawford 44/5”` → `RRC 44/5` and the box's `aria-invalid` cleared; changing the box to `Price 23` over a shown card → card hidden, "Reference changed" prompt, fields Price / 23; `RIC I² Nero 306` → `RIC I (second edition) Nero 306`, term `Nero 306`; 400 px; console clean. Pass.
