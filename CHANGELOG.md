...
- The workspace's **Search** route says in plain words what your saved comparables add up to — `3 comparables · median €180.00 · middle half €150.00–€300.00 · 2024–2026`, then what was left out and why (`Left out: 1 in another currency.`) — where it showed a strip of terms such as `No headline median`, `Insufficient evidence` and `unavailable: manual, authorized-import`; with none saved it says so in one sentence. The set list names each set by its reference with how many it holds (`RIC 27b (4)`), and choosing a set shows it in the currency most of its sales are in; the others are still left out, never converted. Each saved sale is headed by its house and lot and dated in your browser's language.
- **Compare coins** names a coin's saved VAT on the premium and platform fee on the hammer beside its other fees, and its estimated total includes them.
- A new auction in the workspace starts in your own time zone, where it started in Europe/London, or — once you type its name — in the zone you gave the same house's last saved auction, with a line saying which auction the zone came from. The house is the auction's name up to its sale number (`E-Sale 130`, `Auction XXV`, `Sale 61`), with case and spacing set aside, so `Roma Numismatics Auction XXV` follows `Roma Numismatics E-Sale 130`; any other name, however alike (`Heritage Europe 78` after `Heritage 3110`), starts in your own zone. A zone you choose yourself is never replaced. Nothing new is stored for this: the saved auctions are what remember it. Each reminder in a coin's **Reminders** tab says when it goes off in your own time, `Tomorrow 8:00 (your time)`, and at the auction's wall time as well when the auction is in another zone, `14:00 Europe/Zurich`.
- The workspace's auction form, now headed **Auction**, picks the time zone from your browser's list of zones, starting on your own, with **Other…** for a name the list lacks, where it was a text box that started on Europe/London. A timed auction's two reminders are chosen as **1 day before**, **1 hour before**, **30 minutes before**, **Custom…** (a number of minutes) or **No reminder**, where they were minute counts. **Save auction** saves at once, with a line under it saying what it will save, where a confirmation dialog used to ask; **Remove event** shows only for an auction already saved. The form says that a date-only auction's reminders go off at their times in the auction's time zone.
- The workspace's **Reminders** tab names the coin's auction and when it closes, lists its reminders in words (`1 day before`, `Previous day at 09:00`) where it showed minute counts, and for an auction with none offers **Add the standard two** (1 day and 1 hour before; the day before and on the day for a date-only auction). A coin with no auction gets **Attach auction**, which takes you to the auction field under **Details**. Under **Outcome**, the note that **Passed** is unavailable while a placed bid is active shows only while it is.
- A coin's **Details** in the workspace keep open only what a lot page asks for — title, reference, auction page, notes and auction — and fold **Auction identity and links** (with the general source URL), **Coin details** and **Sourced provenance** into sections that open by themselves when they hold something, or when a captured page filled them. A value the browser cannot accept in a folded section — a photo address that is not one, a weight over 1,000 g — opens that section and is named beside **Save details** (`Check Photo URL 1 under Coin details: …`), where Save used to do nothing at all; a value the store refuses, such as a `javascript:` photo address, is named the same way and its section opened. **Save details**, **Undo last edit** and **Remove coin** sit in a bar that stays at the bottom of the window and says **Unsaved changes** while there are any; **Save outcome** under **Outcome** does the same.
...
- The workspace's text boxes hold exactly what a saved record can: a coin's title and an auction's or group's name 300 characters, its reference, lot number, house and sale 120, and collection notes 5,000. The coin form used to take 500 characters where the store kept 120 or 300, so a long pasted title or reference was refused only at save with a field path for a message; collection notes stopped at 2,000 though 5,000 are kept. Near the end a box says how many characters are left.
- The bid calculator under the workspace's **Bid** tab follows a bid you have just saved for the coin, in its currency, amount and premium; it used to keep the figures it first opened with until you chose another coin. What you type in the calculator is still left alone while the coin's saved bid stays the same.
- After you record a placed bid, the workspace's **Bid** tab shows the placed amount, where it went on showing the plan you had saved before; recording the placed bid now moves the plan into the coin's bid history instead of keeping it beside the bid in force. After you record a cancellation the form offers the cancelled amount, currency and premium for you to save again or change; no plan is written for them. On a won, lost or passed coin the **Bid** tab's fields and buttons are disabled and it says to re-open the lot under **Outcome** to change bids, where it offered **Save plan** and **Record placed bid** for the store to refuse.
