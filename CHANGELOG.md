...
- A new auction in the workspace starts in your own time zone, where it started in Europe/London, or — once you type its name — in the zone you gave the same house's last saved auction (the first word of the name, so `Leu Web Auction 32` follows `Leu Web Auction 30`), with a line saying which auction the zone came from. A zone you choose yourself is never replaced. Nothing new is stored for this: the saved auctions are what remember it. Each reminder in a coin's **Reminders** tab says when it goes off in your own time, `Tomorrow 8:00 (your time)`, and at the auction's wall time as well when the auction is in another zone, `14:00 Europe/Zurich`.
