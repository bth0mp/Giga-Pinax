...
- A list of types to choose from (`RIC 237` offers 47) is grouped by volume under small headings with their counts, shows each ruler or mint in bold, says once that the types come from the local catalogue instead of on every row, and past twelve rows offers a filter. The list stands outside **Refine reference**, which now stays closed.
- The popup header draws **Sources** as a text button with a ⌄, like **Panel** and **Settings** beside it, and its menu stays whole on screen at 320 px. The light and dark switch is back as an icon button at the end of the header; it sets the same theme **Settings** → **Appearance** shows, and the last choice made wins.
- The bid calculator's “Enter an amount and buyer premium.” is a muted prompt until there is a result, and its explanation of fees and increments folds under **How the total is counted**.
- The popup shows the coin before its prices. The type card now comes first and the auction research under it; while acsearch answers, the median's place and the filter switch above it are held with “Fetching acsearch…”, so prices arriving a moment later move neither the card nor the median.
...
- A capture of a page that cannot be read no longer opens an empty Ruler/Denomination/Mint/Reference editor or says why twice: the reason shows once, under **Capture current page**. The popup's status line now appears over the top of the panel, just under the Research, Calculator and Watchlist tabs, instead of under the footer; it moves nothing and clears itself after a few seconds.
- The popup's footer fits on one line in the toolbar popup: the credit drops its “Type data:” prefix but still names every catalogue, and the arrow of **acsearch ↗** no longer drops under the word at any width.
