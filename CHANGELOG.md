...
- Damaged settings, a damaged reminder schedule, damaged unsaved drafts or a damaged count of saved changes no longer lock you out of every record, down to opening the workspace: the schedule, the drafts and the count start again, and settings that cannot be read are set aside whole under **Data health** in Settings (where they can be downloaded) while Settings starts afresh from your last default currency. No coin, auction or collection record is touched. Set-aside settings, and an unreadable entry of the list itself, no longer show a **Restore** button that could never work.
