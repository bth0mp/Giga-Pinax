...
- Changing the default currency in Settings, or importing records that carry another one, reaches the next popup you open: it no longer prices once in the old currency and then shows an empty panel. A window already showing prices re-prices them in the new currency where acsearch access is granted, and asks for nothing where it is not.
- **Settings → Updates** is hidden where a store keeps the extension up to date, and every package now carries the MIT licence at its root as `LICENSE.txt`.
...
- A crafted backup can no longer stop every reminder. A revision no write could have counted to, or an auction whose reminder falls outside the instants a record can hold, passed the import and then made every later reconcile fail silently; both are refused now, a store that already holds one still opens with all its records, and a reconcile that fails says so on the toolbar.
...
- The extension package grows from about 1.87 MB (0.31.1's released ZIP: 1,872,932 bytes) to about 2.48 MB, mostly for the bundled Crawford, Price and Seleucid Coins records, their labels and the RIC number index.
