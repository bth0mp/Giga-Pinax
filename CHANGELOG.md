...
- A grade median and a year median read as one line of the ledger, the label, the median and the sales it rests on: `VF · $260 · 3 sales`, `2023 · $200 · 3 sales`, in the panel and in **Copy summary**, where they read `VF: median $260 (3)` before.

### Fixed

- The grade medians read the spellings whole houses grade in, which were dropped as ungraded before: NAC's `Fdc`, the British `GVF`, `GEF`, `NEF`, `NVF` and `nEF`, the German `Stgl.`, `fast Stgl.`, `prfr.`, `prägefrisch` and `sge`, the American `BU`, `aUNC` and `NGC Gem MS`, the Italian `Spl`, the Spanish `S/C` and the English `Fair` (counted under Fine and below), with `Prfr.`, `Gem BU` and the Spanish label `Conservación:`. `S/C` is read where it opens the text; where it follows a sentence that ends in a number, a capitalised word other than a side label such as `Rev.`, a weight (`27,23 g. S/C.`) or one of the closing remarks `original`, `bella`, `pátina`, `brillo`, `rara`, `escasa` and `atractiva`; behind a qualifier or `Conservación:`; and where it is joined to another grade. Behind a sentence describing the type in lower-case words (`Roma sentada. S/C.`) it stays the senate's mark. `BU` in a monogram and a lower-case "fair" stay unread.
- A pasted or right-clicked lot reads the ruler its heading names in Latin, German, French, Italian or Spanish: `Traianus`, `Trajano`, `Traiano`, `Nerón`, `Néron`, `Nerone`, `Adriano`, `Hadrien`, `Vespasiano`, `Vespasien`, `Philippus I`, `Valerianus I`, `Elagabal`, `Heliogabalus`, `Faustina II` / `Minor` / `Maior`, `Constantius I`, `Maximinus II`, `Julian II`, `Jovian`, `Constantine the Great`, `Konstantin I` and `Costantino I` among them, and the second of a joint heading (`Philipp I. und Philipp II.`, `Costantino I per Costantino II`). A Künker lot headed "Traianus, 98-117" citing `RIC 347` now offers Trajan's two types instead of thirty-four from every volume, and a Spanish "Nerón … Roma. RIC 53" opens Nero's denarius instead of listing the Rome mint's folles. Each spelling names one person Nomisma already knows; "the stone of Elagabal", "Sol Elagabal" and "Piedra sagrada de Elagabal" on a coin of Uranius Antoninus name the god, not a second emperor, and "Jovian eagle" is the adjective.
- A lot heading that names a ruler and a mint (`Constantius I. Follis. Trier. RIC VI 12.`, `Constantius Chlorus. Follis. London. RIC VI 14.`) no longer opens the ruler's coin from another RIC VI–IX mint when his number is not his at the named one: that coin is offered, locally and online. A coin of the named mint still opens.
- `Bopearachchi & Rahman 268` (the co-authored *Pre-Kushana Coins in Pakistan*, also written `Bopearachchi and Rahman` or `Bopearachchi-Rahman`, with a hyphen or a dash) is read as its own citation and searched for prices only, no longer as Bopearachchi 268 of a king called "& Rahman". The French `Bopearachchi Série 6C` (or `Séries`) is Bop 6C with no king, where "Série" was taken for the king and then required as a word on acsearch.
- The provenance a lot or a captured page states is read in German, Italian, Spanish and French as it is in English: `Ex Slg. Dr. X, erworben 1988 bei Lanz` is one entry, no longer cut to "Ex Slg", and a sentence still ends at the full stop a catalogue citation follows, even behind an initial (`Aus Sammlung Dr. W. R. RIC 53.` keeps its RIC 53); `Nr. 1234`, `Los`, `lotto`, `lote`, `n° 123`, `nº 123` and the Italian `n. 45` give the lot, and a year followed by a comma (`Zürich 2000, Nr. 12`) is read; entries opening `Exemplar der Auktion`, `Aus Sammlung`, `Erworben`, `Provenienz:`, `Provient de` and `Proviene da` are found, with their Spanish, Italian and French dates. A `no.` or `Nr.` before a sale's number (`Auction no. 45, lot 12`) is still never taken for the lot.
- A lot dated after today is never counted as a sale on the acsearch panel, even when its price field holds a figure such as a starting price: it is left out of the median, the range, the grade and year medians, the trend and the last sale, is listed under **Upcoming** instead, and **Copy summary** says `1 future-dated lot not counted`. The panel's coverage line already counts it among the results not counted. CoinArchives rows dated after today were already left out.
- **Only results citing …** recognises three more ways a dealer writes the citation: a hyphen for Crawford's slash (`Crawford 344-1a`, where it cannot be a shortened range such as `44-5`), a RIC volume in Arabic figures with its part (`RIC 2.1 356`, but never `RIC 2 356`), and full stops between key, volume, part and number (`RIC.I.53`, `RIC.II.53`, `RIC.II.1.53`). A part is one figure, so a figure glued behind the numeral with another number after it (`RIC VI.1 53`) is still read as the part and the last number as the type, never the other way round.
